  	 ...
  	 public function checklogin()
  {
     ...
     if(!$_SESSION['username'])
          {
                ...
                echo "location.href='./wagent.php?c=site&a=entry&op=display&do=login&m=weixinmao_house&i=4'";
	...
	public function doWebLogin() {
      ...
      if($_SESSION['username'])
          {
                ...
                echo "location.href='./wagent.php?c=site&a=entry&op=display&do=main&m=weixinmao_house&i=4'";
      ...
      if($_POST)
      {
			...
			$sql = 'SELECT * FROM ' . tablename('weixinmao_house_agent') . ' WHERE `uniacid` = :uniacid AND `username` =:username  AND `password` =:password';
			$agentinfo = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'],':username'=>$username,':password'=>$password));
			...
			if($agentinfo)
			{
              ...
              if($agentinfo['enabled'] ==1)
						{
                
                $_SESSION['username'] = $username;
                
                $_SESSION['loginid'] = $agentinfo['id'];
                
				$list = array('msg'=>'登录成功','error'=>0);
		...
		include $this->template('login', TEMPLATE_INCLUDEPATH, true);
	}
	...
	public function doWebMain() {
		...
		load()->func('tpl');
     	...
     	$agentinfo = pdo_get('weixinmao_house_agent',array('id'=>$loginid));
		...
		if ($operation == 'post') {
			...
			$psize = 15;
			$condition = ' WHERE `uniacid` = :uniacid AND uid = :uid AND ispub =0';
			$params = array(':uniacid' => $_W['uniacid'],':uid'=>$agentinfo['uid']);
			...
			message('删除成功！', './wagent.php?c=site&a=entry&op=display&do=main&m=weixinmao_house&i=4', 'success');
		}
      ...
      include $this->template('main', TEMPLATE_INCLUDEPATH, true);
	}
  	...
  	public function doWebAgentlethouse() {
		...
		if ($operation == 'post') {
			...
			//	exit;
			  ...
			  // exit;
                ...
                message('更新成功！', $this->createWebUrl('agentlethouse', array('op' => 'display')), 'success');
            }
		...
		include $this->template('agentlethouse');
...
public function doWebPaylist() {
	...
	public function doWebCate() {
  	...
  	public function doWebAgentrole() {
		global $_W, $_GPC;
	    load()->func('tpl');
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		if ($operation == 'display') {
			$list = pdo_fetchall("SELECT * FROM " . tablename('weixinmao_house_agentrole') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY sort DESC");
		} elseif ($operation == 'post') {
			$id = intval($_GPC['id']);
			...
			if (checksubmit('submit')) {
				$data = array(
					...
					'money' => $_GPC['money'],
				    'days'=>$_GPC['days'],
					'lethousenum'=>$_GPC['lethousenum'],
					'oldhousenum'=>$_GPC['oldhousenum'],
					'isinit'=>$_GPC['isinit'],
					'sort' => $_GPC['sort'],
					'enabled'=>1
					);
				if (!empty($id)) {
					pdo_update('weixinmao_house_agentrole', $data, array('id' => $id));
				} else {
					pdo_insert('weixinmao_house_agentrole', $data);
					$id = pdo_insertid();
				}
				message('更新成功！', $this->createWebUrl('agentrole', array('op' => 'display')), 'success');
			}

			$toplist = pdo_fetch("select * from " . tablename('weixinmao_house_agentrole') . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W['uniacid']));
		   ...
		   $companyrole = pdo_get('weixinmao_house_agentrole',array('uniacid'=>$_W['uniacid'],'id'=>$roleid));
		 ...
		 // pdo_update('weixinmao_zp_company',$companydata,array('uniacid'=>$_W['uniacid']));
		  // pdo_update('weixinmao_zp_job',$data,array('uniacid'=>$_W['uniacid']));
			...
			message('同步成功！', $this->createWebUrl('agentrole', array('op' => 'display')), 'success');


		}elseif ($operation == 'delete') {
			$id = intval($_GPC['id']);
			$oldhouseprice = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_agentrole') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
			if (empty($oldhouseprice)) {
				message('抱歉，不存在或是已经被删除！', $this->createWebUrl('agentrole', array('op' => 'display')), 'error');
			}
			pdo_delete('weixinmao_house_agentrole', array('id' => $id));
			message('删除成功！', $this->createWebUrl('agentrole', array('op' => 'display')), 'success');
		} else {
			message('请求方式不存在');
		}
		include $this->template('agentrole', TEMPLATE_INCLUDEPATH, true);
	}
	...
	public function doWebHousemsg() {
		...
		if ($operation == 'post') {
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_shopsaleinfo') .$condition ;
		
			$total = pdo_fetchcolumn($sql, $params);
			
			if (!empty($total)) {
				$sql = 'SELECT * FROM  ' . tablename('weixinmao_house_shopsaleinfo') .$condition.' ORDER BY  `createtime`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_buesaleinfo') .$condition ;
		
			$total = pdo_fetchcolumn($sql, $params);
			
			if (!empty($total)) {
				$sql = 'SELECT * FROM  ' . tablename('weixinmao_house_buesaleinfo') .$condition.' ORDER BY  `createtime`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_carsaleinfo') .$condition ;
		
			$total = pdo_fetchcolumn($sql, $params);
			
			if (!empty($total)) {
				$sql = 'SELECT * FROM  ' . tablename('weixinmao_house_carsaleinfo') .$condition.' ORDER BY  `createtime`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
		...
		} elseif($operation == 'doneshop'){
			
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_shopsaleinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，订单不存在或是已经被删除！');
			}
			 pdo_update('weixinmao_house_shopsaleinfo', array('ischeck'=>1), array('id' => $id));

			message('操作成功！', referer(), 'success');
			
		
		} elseif($operation == 'donecar'){
			
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_carsaleinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，订单不存在或是已经被删除！');
			}
			 pdo_update('weixinmao_house_carsaleinfo', array('ischeck'=>1), array('id' => $id));

			message('操作成功！', referer(), 'success');
			
		
		} elseif($operation == 'donebue'){
			
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_buesaleinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，订单不存在或是已经被删除！');
			}
			 pdo_update('weixinmao_house_buesaleinfo', array('ischeck'=>1), array('id' => $id));
		...
		}elseif($operation == 'shopdetail'){
			
			$id = intval($_GPC['id']);
			$saleinfo = pdo_fetch("SELECT * FROM " . tablename('weixinmao_house_shopsaleinfo') . " WHERE id = :id", array(':id' => $id));
		...
		}elseif($operation == 'cardetail'){
			
			$id = intval($_GPC['id']);
			$saleinfo = pdo_fetch("SELECT * FROM " . tablename('weixinmao_house_carsaleinfo') . " WHERE id = :id", array(':id' => $id));
		...
		}elseif($operation == 'buedetail'){
			
			$id = intval($_GPC['id']);
			$saleinfo = pdo_fetch("SELECT * FROM " . tablename('weixinmao_house_buesaleinfo') . " WHERE id = :id", array(':id' => $id));
           ...
           $piclist1 = unserialize($saleinfo['thumb_url']);
			...
			pdo_delete('weixinmao_house_buesaleinfo', array('id' => $id));

			message('删除成功！', referer(), 'success');
		}elseif ($operation == 'deleteshop') {
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_shopsaleinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，不存在或是已经被删除！');
			...
			pdo_delete('weixinmao_house_shopsaleinfo', array('id' => $id));

			message('删除成功！', referer(), 'success');
		}elseif ($operation == 'deletecar') {
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_carsaleinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，不存在或是已经被删除！');
			...
			pdo_delete('weixinmao_house_carsaleinfo', array('id' => $id));
	...
	public function doWebBusinesshouse() {
		...
		if ($operation == 'post') {
			...
			pdo_delete('weixinmao_house_businesshouseinfo', array('id' => $id));
	...
	public function doWebOldhouse() {
			...
			//	exit;
                ...
                $data = array(
                  ...
                  'fxmoney'=>$_GPC['fxmoney'],
					...
					'status'=>$_GPC['status'],
					'bid'=>$_GPC['bid'],
                  'finalsale'=>$_GPC['finalsale'],
                  'userthink'=>$_GPC['userthink'],
  	...
  	public function doWebShophouse() {
		//这个操作被定义用来呈现 管理中心导航菜单
		global $_GPC, $_W;
		load()->func('tpl');
		...
		if ($operation == 'post') {
			...
			$id = $_GPC['id'];
			 ...
			 if (!empty($id)) {
				 		...
				 		$item = pdo_fetch("SELECT *  FROM " . tablename('weixinmao_house_shophouseinfo') . " WHERE id = :id", array(':id' => $id));
				 		$sql = 'SELECT * FROM ' . tablename('weixinmao_house_area') . ' WHERE `uniacid` = :uniacid AND `cityid`=:cityid ORDER BY `sort` DESC';
						$arealist = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'],':cityid'=>$item['cityid']));
					   $sql = 'SELECT * FROM ' . tablename('weixinmao_house_buildarea') . ' WHERE `uniacid` = :uniacid AND `aid`=:aid ORDER BY `sort` DESC';
					    $buildarealist = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'],':aid'=>$item['houseareaid']));
			...
			//	exit;
			  ...
			  // exit;
                if (!empty($id)) {
                    unset($data['createtime']);
                    pdo_update('weixinmao_house_shophouseinfo', $data, array('id' => $id));
                } else {
                    pdo_insert('weixinmao_house_shophouseinfo', $data);
                    $id = pdo_insertid();
                }
                message('更新成功！', $this->createWebUrl('shophouse', array('op' => 'display')), 'success');
            }
			 ...
			 if (!empty($id)) {
				 		...
				 		$item = pdo_fetch("SELECT *  FROM " . tablename('weixinmao_house_shoplethouseinfo') . " WHERE id = :id", array(':id' => $id));
			...
			//	exit;
			  ...
			  // exit;
                if (!empty($id)) {
                    unset($data['createtime']);
                    pdo_update('weixinmao_house_shoplethouseinfo', $data, array('id' => $id));
                } else {
                    pdo_insert('weixinmao_house_shoplethouseinfo', $data);
                    $id = pdo_insertid();
                }
                message('更新成功！', $this->createWebUrl('shophouse', array('op' => 'display')), 'success');
            }
			...
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_shophouseinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，订单不存在或是已经被删除！');
			}
			 pdo_update('weixinmao_house_oldhouseinfo', array('ischeck'=>1), array('id' => $id));
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_shophouseinfo') .$condition ;
		
			$total = pdo_fetchcolumn($sql, $params);
			
			if (!empty($total)) {
				$sql = 'SELECT * FROM  ' . tablename('weixinmao_house_shophouseinfo') .$condition.$sort.'  LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_shoplethouseinfo') .$condition ;
		
			$total = pdo_fetchcolumn($sql, $params);
			
			if (!empty($total)) {
				$sql = 'SELECT * FROM  ' . tablename('weixinmao_house_shoplethouseinfo') .$condition.$sort.' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
			...
			pdo_delete('weixinmao_house_shophouseinfo', array('id' => $id));

			message('删除成功！', referer(), 'success');
		}elseif ($operation == 'deletelet') {
			$id = intval($_GPC['id']);
			$row = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_shoplethouseinfo') . " WHERE id = :id", array(':id' => $id));
			if (empty($row)) {
				message('抱歉，不存在或是已经被删除！');
			}

			pdo_delete('weixinmao_house_shoplethouseinfo', array('id' => $id));

			message('删除成功！', referer(), 'success');
		}
	
		include $this->template('shophouse');
	...
	public function doWebAdv() {
  	...
  	public function doWebAdvmoney() {
		global $_W, $_GPC;
			load()->func('tpl');
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		if ($operation == 'display') {
			$list = pdo_fetchall("SELECT * FROM " . tablename('weixinmao_house_advmoney') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id DESC");
		} elseif ($operation == 'post') {
			$id = intval($_GPC['id']);
			if (checksubmit('submit')) {
				$data = array(
				...
				if (!empty($id)) {
					pdo_update('weixinmao_house_advmoney', $data, array('id' => $id));
				} else {
                 
					pdo_insert('weixinmao_house_advmoney', $data);
					$id = pdo_insertid();
				}
				message('更新成功！', $this->createWebUrl('advmoney', array('op' => 'display')), 'success');
			}
			$adv = pdo_fetch("select * from " . tablename('weixinmao_house_advmoney') . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W['uniacid']));
		} elseif ($operation == 'delete') {
			$id = intval($_GPC['id']);
			$adv = pdo_fetch("SELECT id FROM " . tablename('weixinmao_house_advmoney') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
			if (empty($adv)) {
				message('抱歉，幻灯片不存在或是已经被删除！', $this->createWebUrl('advmoney', array('op' => 'display')), 'error');
			}
			pdo_delete('weixinmao_house_advmoney', array('id' => $id));
			message('幻灯片删除成功！', $this->createWebUrl('advmoney', array('op' => 'display')), 'success');
		} else {
			message('请求方式不存在');
		}
		include $this->template('advmoney', TEMPLATE_INCLUDEPATH, true);
	}
...
public function doWebToplist() {
	...
	public function doWebAgent() {
      ...
      $sql = 'SELECT * FROM ' . tablename('weixinmao_house_houseinfo') . ' WHERE `uniacid` = :uniacid ORDER BY `sort` DESC , createtime DESC';
		$newhouselist = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
		...
		if ($operation == 'display') {
			...
			if (!empty($total)) {
				...
				foreach($list as $k=>$v)
                        ...
                        $agentrole = pdo_get('weixinmao_house_agentrole',array('id'=>$v['roleid']));

								$list[$k]['agentrolename'] =  $agentrole['title'];
          ...
          $pindex = max(1, intval($_GPC['page']));
			$psize = 15;
			$condition = ' WHERE `uniacid` = :uniacid  AND tid=0 ';
			$params = array(':uniacid' => $_W['uniacid']);
				...
				foreach($list as $k=>$v)
						{
                            ...
                            $myteamlist = pdo_getall('weixinmao_house_agent',array('uniacid'=>$_W['uniacid'],'tid'=>$v['uid']));
							
							$des = '团队:'.count($myteamlist).'人';
							
							$list[$k]['mark'] = $des;
							
							$list[$k]['teamlist'] = $myteamlist;
          ...
          $pindex = max(1, intval($_GPC['page']));
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('weixinmao_house_housemsg') .$condition ;
				...
				foreach($list as $k=>$v)
						{
                  				...
                  				$agentinfo = pdo_get('weixinmao_house_agent',array('uniacid'=>$_W['uniacid'],'uid'=>$v['agentid']));
			...
			$agent = pdo_fetch("select * from " . tablename('weixinmao_house_agent_setting') . " where  uniacid=:uniacid limit 1", array(":uniacid" => $_W['uniacid']));
			...
			if (checksubmit('submit')) {
             ...
             // exit;
				if ($agent) {
					...
					pdo_update('weixinmao_house_agent_setting', $data, array('id' => $agent['id']));
				} else {
					pdo_insert('weixinmao_house_agent_setting', $data);
					$id = pdo_insertid();
				}
				message('更新成功！', $this->createWebUrl('agent', array('op' => 'agent_setting')), 'success');
			}
			...
			if (checksubmit('submit')) {
			   ...
			   $agentroleid = $_GPC['agentroleid'];
			   
			       
        $agentrole = pdo_get('weixinmao_house_agentrole',array('uniacid'=>$_W['uniacid'],'id'=>$agentroleid));
		...
		$data = array(
			...
			$agentrolelist = pdo_fetchall("select * from " . tablename('weixinmao_house_agentrole') . " where  uniacid=:uniacid ", array(":uniacid" => $_W['uniacid']));
	...
	public function doWebCoupon() {
		...
		if ($operation == 'display') {
            ...
            $id = intval($_GPC['id']);
			$oldhouseprice = pdo_fetch("SELECT id,status FROM " . tablename('weixinmao_house_fxmessage') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
			if (empty($oldhouseprice)) {
				message('抱歉，不存在或是已经被删除！', $this->createWebUrl('coupon', array('op' => 'fxmessage')), 'error');
			}
            
      
          	if($oldhouseprice['status'] == 1)
            {
          	  $fxrecord = pdo_getall('weixinmao_house_fxrecord', array('orderid' => $id));
