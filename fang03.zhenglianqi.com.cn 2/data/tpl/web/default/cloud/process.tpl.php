			...
			if(!path && i >= total) {
				if ($scope.fails && $scope.fails.length > 0) {
					...
					if (config.type == 'theme') {
						...
						targetUrl = './index.php?c=cloud&a=process&step=schemas&m=' + config.appname + '&is_upgrade=' + config.is_upgrade + '&batch=1&support=' + config.account_type + '&application_type=' + config.application_type +  '&has_new_support=<?php  echo $has_new_support;?>';
					}
...
<div class="clearfix js-processor" ng-controller="FileProcessorCtrl" ng-cloak>
	...
	<script type="text/javascript">
		angular.module('cloudApp').value('config', {
			...
			application_type: '<?php  echo $application_type;?>',
			type : '<?php  echo $type;?>',
			appname : '<?php  echo $m;?>',
...
<div class="clearfix js-processor" ng-cloak ng-controller="SchemasProcessorCtrl">
	<?php  if(empty($packet['schemas'])) { ?>
		...
		<?php  if(!empty($packet['install'])) { ?>
			<script>
				location.href = '<?php  echo url("module/manage-system/install", array("module_name" => $m, "flag" => 1, "support" => $_GPC["support"], "application_type" => $_GPC["application_type"]));?>';
			</script>
		<?php  } ?>
		<!-- 升级 -->
		<script>
			<?php  if($_GPC['batch']) { ?>
				location.href = '<?php  echo url("cloud/process", array("step" => "scripts", "m" => $m, "is_upgrade" => $is_upgrade, "batch" => 1, "support" => $_GPC["support"], "has_new_support" => $has_new_support, "application_type" => $_GPC["application_type"]));?>';
			<?php  } else { ?>
				location.href = '<?php  echo url("cloud/process", array("step" => "scripts", "m" => $m, "is_upgrade" => $is_upgrade, "support" => $_GPC["support"], "has_new_support" => $has_new_support, "application_type" => $_GPC["application_type"]));?>';
			<?php  } ?>
		</script>
	<?php  } ?>
	<div class="upgrade-pragress we7-margin-bottom">
			...
			<div class="modal-content">
				...
				<div class="modal-body overflow-auto">
					...
					<div class="we7-empty-blcok we7-margin-bottom-sm">
						<?php  if(is_array($packet['tips'])) { foreach($packet['tips'] as $tip) { ?>
						<p><?php  echo $tip['version'];?>更新微擎新模式详情：</p>
						<div><?php  echo $tip['message'];?></div>
						<?php  } } ?>
					</div>
				</div>
