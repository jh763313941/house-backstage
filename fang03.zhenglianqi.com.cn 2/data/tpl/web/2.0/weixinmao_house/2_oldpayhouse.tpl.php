...
<ul class="nav nav-tabs">
	<li <?php  if($operation == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('oldpayhouse', array('op' => 'post'))?>">添加拍卖房源</a></li>
	<li <?php  if($operation == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('oldpayhouse', array('op' => 'display'))?>">拍卖房源列表</a></li>
	...
	<form action="" method="post" class="form-horizontal form" enctype="multipart/form-data" id="form1" onsubmit='return formcheck()'>
		<div class="panel panel-default">
			<div class="panel-heading">
				<?php  if(empty($item['id'])) { ?>添加拍卖房源<?php  } else { ?>编辑拍卖房源<?php  } ?>
			</div>
			<div class="panel-body">
				
				<div class="tab-content">
					<div class="tab-pane  active" id="tab_basic"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('oldpayhouse_basic', TEMPLATE_INCLUDEPATH)) : (include template('oldpayhouse_basic', TEMPLATE_INCLUDEPATH));?></div>
...
<div class="main">
	...
	<div class="panel-body">
		<form action="./index.php" method="get" class="form-horizontal" role="form">
			...
			<input type="hidden" name="do" value="oldpayhouse" />
			...
			<tbody>
				...
				<tr>
					...
					<td style="text-align:right;">
						<a href="<?php  echo $this->createWebUrl('oldpayhouse', array('id' => $item['id'], 'op' => 'post'))?>"class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="编辑"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
						<a href="<?php  echo $this->createWebUrl('oldpayhouse', array('id' => $item['id'], 'op' => 'delete'))?>" onclick="return confirm('此操作不可恢复，确认删除？');return false;" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="删除"><i class="fa fa-times"></i></a>
					</td>
				</tr>
				<?php  } } ?>
			</tbody>
