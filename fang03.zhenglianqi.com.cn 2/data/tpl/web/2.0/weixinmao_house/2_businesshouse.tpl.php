...
<ul class="nav nav-tabs">
	<li <?php  if($operation == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('businesshouse', array('op' => 'post'))?>">添加商业地产</a></li>
	<li <?php  if($operation == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('businesshouse', array('op' => 'display'))?>">管理列表</a></li>
	...
	<form action="" method="post" class="form-horizontal form" enctype="multipart/form-data" id="form1" onsubmit='return formcheck()'>
		<div class="panel panel-default">
			<div class="panel-heading">
				<?php  if(empty($item['id'])) { ?>添加商业地产<?php  } else { ?>编辑商业地产<?php  } ?>
			</div>
			<div class="panel-body">
				
				<div class="tab-content">
					<div class="tab-pane  active" id="tab_basic"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('businesshouse_basic', TEMPLATE_INCLUDEPATH)) : (include template('businesshouse_basic', TEMPLATE_INCLUDEPATH));?></div>
