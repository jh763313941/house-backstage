...
<ul class="nav nav-tabs">
	<li <?php  if($operation == 'display') { ?> class="active" <?php  } ?>><a href="<?php  echo $this->createWebUrl('advmoney',array('op' =>'display'))?>">收益广告管理</a></li>
	<li<?php  if(empty($adv['id']) && $operation == 'post') { ?> class="active" <?php  } ?>><a href="<?php  echo $this->createWebUrl('advmoney',array('op' =>'post'))?>">添加收益广告</a></li>
	<?php  if(!empty($adv['id']) &&  $operation == 'post') { ?><li  class="active"><a href="<?php  echo $this->createWebUrl('advmoney',array('op' =>'post','id'=>$adv['id']))?>">编辑收益广告</a></li><?php  } ?>
</ul>
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
                  ...
                  <td><?php  if($adv['type']==0) { ?><span class='label label-success'>首页</span> <?php  } ?><?php  if($adv['type']==1) { ?><span class='label label-success'>房源内页</span> <?php  } ?></td>
		...
		<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span style="color:red">*</span>广告位置</label>
          <div class="col-sm-8 col-xs-12">
              <select class="form-control tpl-category-parent we7-select" id="type" name="type">
                      <option value="0" <?php  if($adv['type'] == 0) { ?> selected <?php  } ?>>首页</option>
                                    
                       <option value="1" <?php  if($adv['type'] == 1) { ?> selected <?php  } ?>>房源内页</option>
				 ...
				 <div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<input type="text" name="advid" class="form-control" value="<?php  echo $adv['advid'];?>" />
					</div>
				</div>
