	...
	foreach ($modulelist as $modulename => $module) {
		...
		if (version_compare($module['version'], $manifest['application']['version']) == '-1') {
			...
			continue;
				...
				if ($manifest['status'] && $manifest['system_shutdown'] == 1 && !empty($manifest['branches']) && $manifest['application_type'] == APPLICATION_TYPE_MODULE) {
		...
		if (!empty($manifest['service_expiretime'])) {
			...
			if ($manifest['status'] && $manifest['system_shutdown'] == 1 && $manifest['service_expiretime'] < time()) {
